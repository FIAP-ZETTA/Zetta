              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="relative" ref={notifRef}>
                  <button
                    ...
                    id="notifications-btn"
                    onClick={() => setShowNotif(v => !v)}
                    className="relative flex h-8 w-8 items-center justify-center border border-border bg-card text-muted-foreground transition-all hover:border-primary/50 hover:text-foreground rounded-lg"
                    aria-label={t.header.notifications}
                  >
                    <Bell className="h-3.5 w-3.5" />
                    {critCount > 0 && (
                      <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center bg-destructive text-[9px] font-bold text-white shadow-[0_0_8px_rgba(244,63,94,0.8)] rounded-full">
                        {critCount > 9 ? "9+" : critCount}
                      </span>
                    )}
                  </button>
                  ...
                  {/* Dropdown notificações */}
                  {showNotif && (
                    <div className="absolute right-0 top-10 z-50 w-80 border border-border bg-card shadow-2xl rounded-xl overflow-hidden animate-fade-up">
                      <div className="flex items-center justify-between border-b border-border px-4 py-2.5 bg-muted/20">
                        <div className="flex items-center gap-2">
                          <p className="text-xs font-bold uppercase tracking-wider text-foreground">{t.header.notifications}</p>
                          {critCount > 0 && (
                            <span className="bg-destructive/15 px-1.5 py-0.2 text-[10px] font-bold text-destructive border border-destructive/30 rounded">
                              {critCount}
                            </span>
                          )}
                        </div>
                        {critCount > 0 && (
                          <button
                            ...
                            className="text-[10px] text-muted-foreground hover:text-primary font-bold transition-colors cursor-pointer hover:underline"
                          >
                            Limpar
                          </button>
                        )}
                      ...
                      {critCount === 0 ? (
                        ...
                        <div className="p-2 space-y-1">
                          <button
                            onClick={() => { setShowNotif(false); router.push("/zettascan") }}
                            className="w-full flex items-start gap-2.5 p-2.5 text-left transition-colors hover:bg-muted rounded-lg"
                          >
                            <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-destructive" />
                            <div>
                              <p className="text-xs font-bold text-foreground">
                ...
                {/* Perfil / Avatar */}
                <div className="flex h-8 w-8 items-center justify-center bg-primary/10 text-xs font-mono font-bold text-primary border border-primary/30 shadow-[0_0_10px_var(--primary-glow)] rounded-lg">
                  ZG
                </div>
              </div>
            </>
